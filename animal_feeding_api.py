...
UPLOAD_DIR = Path("uploads/animal_feeding")
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="Animal Feeding Verification API", version="1.0.0")
...
def verify_animal_feeding_activity(image_path):
    """Verify animal feeding activity in the image"""
    try:
        if animal_model is None:
            return {
                'is_valid': False,
                'detected_classes': [],
                'message': "Animal feeding model not loaded"
            }
        
        image = cv2.imread(str(image_path))
        results = animal_model(image)
        ...
        if not person_detected:
            return {
                ...
                'message': "No person detected in animal feeding activity"
            }
        ...
        if not animal_detected:
            return {
                'is_valid': False,
                'detected_classes': detected_classes,
                'message': "No animal feeding activity detected in image"
            }
        
        return {
            'is_valid': True,
            'detected_classes': detected_classes,
            'message': f"Animal feeding activity verified successfully"
        }
...
async def startup_event():
    """Initialize the animal feeding model on startup"""
    global animal_model, ai_detector
    ...
    try:
        ...
        print(f"❌ Failed to initialize animal feeding system: {e}")
        animal_model = None
        ai_detector = None
...
async def root():
    return {
        "message": "Animal Feeding Verification API",
        "status": "running" if animal_model is not None else "error",
        "model_loaded": animal_model is not None
    }

@app.post("/verify-animal-feeding", response_model=VerificationResponse)
async def verify_animal_feeding(task_image: UploadFile = File(...)):
    """Verify animal feeding task submission"""
    try:
        if animal_model is None:
            raise HTTPException(status_code=503, detail="Animal feeding model not loaded")
        
        # Save uploaded image
        image_path = UPLOAD_DIR / f"animal_{uuid.uuid4()}.jpg"
        ...
        # Step 3: Animal Feeding Activity Verification
        activity_result = verify_animal_feeding_activity(image_path)
        ...
        results['message'] = "Animal feeding task verified successfully!"
